...
function checkEyesInValidationBox() {
  if (faceFeedbackBox != null && latestEyeFeatures) {
    ...
    if (xPositions && yPositions) {
      faceFeedbackBox.style.border = "10px solid blue";
      faceFeedbackBox.style.borderRadius = "500px";
    } else {
      faceFeedbackBox.style.border = "10px solid blue";
      faceFeedbackBox.style.borderRadius = "500px";
    }
...
async function semiLoop() {
  if (!paused) {
    ...
    requestAnimationFrame(semiLoop);
  }
}
...
async function init(stream) {
  ...
  videoContainerElement.style.zIndex = 50000;
...
webgazer.begin = function (onFail) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      console.log("webgazer begin: error", err);
      onFail();
      console.log("webgazer begin: onFail done");
      videoElement = null;
...
webgazer.semiPause = async function () {
  paused = false;
  document.removeEventListener("click", clickListener, true);
  document.removeEventListener("mousemove", moveListener, true);
  await semiLoop();
...
webgazer.applyKalmanFilter = function (val) {
  ...
  console.log("applyKalmanFilter", val);
  return webgazer;
};
...
webgazer.setTracker = function (name) {
  ...
  console.log("set tracker to " + name);
  return webgazer;
};
...
webgazer.setRegression = function (name) {
  ...
  regs[0].setData(data);
  console.log("set regression to " + name);
  return webgazer;
};
